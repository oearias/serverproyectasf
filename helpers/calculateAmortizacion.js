...
const devuelve_grand_total = async (credito) => {
    ...
    try {
        ...
        //fechaHoy = new Date(fechaHoy.getFullYear(), fechaHoy.getMonth(), fechaHoy.getDate() + 15 );
        let fechaToCompare;
        let monto_total = Number(credito.monto_total);

        const cliente_cump = await pool.query(`SELECT fu_get_cliente_cumplido(${credito.id})`);
        const {fu_get_cliente_cumplido} = cliente_cump.rows[0];
        const cliente_cumplido = fu_get_cliente_cumplido;
        ...
        ////En este bloque calculamos la semana del año, para eso tomamos la fecha inicial y un dia deespues para que solo delimite una semana del resultado de la query
        //Esto evita que se traslapen las fechas
        let fechaWeekyear = new Date(credito.fecha_inicio_real);
        ...
        //Hacemos el For de la amortizacion
        for (let i = 0; i < credito.num_semanas; i++) {
            ...
            let fecha = new Date(credito.fecha_inicio_real);
            let fechac1 = fecha;
            fecha.setDate(fecha.getDate() + (i * 7));
            fecha = fecha.toISOString().slice(0, 10);
            ...
            //Preguntamos si existen bonificaciones para cambiar montos a pagar semanalmente
            //Esto solo si ya es la ultima vuelta
            const descuento = credito.monto_semanal / 2;

            if( num_semana === credito.num_semanas && (credito.inversion_positiva || cliente_cumplido ) ){
                
                if( (credito.inversion_positiva && !cliente_cumplido) || (cliente_cumplido && !credito.inversion_positiva) ){
                    credito.monto_semanal = credito.monto_semanal / 2;
                }

            }

            //Preguntamos si se cubrió el pago
            if (suma_monto_pagado >= credito.monto_semanal) {
                pago_cubierto = 1;
                ...
                if (rows.length > 0) {
                    ...
                    for (let i = 0; i < lista_pagos.rows.length && monto_pagado_semanal < credito.monto_semanal; i++) {

                        monto_pagado_semanal = Number(monto_pagado_semanal) + Number(lista_pagos.rows[i]['monto']);

                        //Preguntamos si se cubre el pago en cada vuelta y calculamos penalizacion
                        if (monto_pagado_semanal >= credito.monto_semanal) {
                            fechaToCompare = lista_pagos.rows[i]['fecha']
                        }
            ...
            //si ya se termino de pagar el credito guardamos en la base de datos
            //que datos necesitamos credito_id, num_semana, fecha, y los totales
            penalizacion_semanal = ( Number(credito.monto_otorgado) * Number(dias_penalizacion)) / 100;


            if(credito.aux_num_penalizaciones === 0){
                penalizacion_semanal = 0;
                dias_penalizacion = 0;
            }

            adeudo_semanal = (credito.monto_semanal + penalizacion_semanal) - suma_monto_pagado;

            penalizacion_total += Number(penalizacion_semanal);
            pagado_total += Number(suma_monto_pagado);

            console.log('La penalizacion de Israel es:', credito.aux_num_penalizaciones);

            //Ultima vuelta, inversion positiva y cliente cumplido
            if (num_semana === credito.num_semanas) {

                if (credito.inversion_positiva && cliente_cumplido) {
                ...
                }else if(credito.inversion_positiva){

                    console.log('Descuento 50% inversion');
                    console.log('monto semanal', credito.monto_semanal);
                    console.log('adeudo semanal',adeudo_semanal);
                    console.log('monto pagado', suma_monto_pagado);
                    console.log('descuento', descuento);
        ...
        grand_total = ( Number(penalizacion_total) + Number(monto_total)) - Number(pagado_total);

        console.log('Monto total:', monto_total);
        console.log('Total pagado:', pagado_total);
        console.log('Total de penalizaciones:', Number(penalizacion_total));
        console.log('Grand Total:', grand_total);

        const formatoMoneda = accounting.formatMoney(grand_total, {symbol: '$', format: '%s %v'});
